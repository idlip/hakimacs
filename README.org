...
│   └── js/
│       └── index.js          # keyboard, menu, search, palette, themes-gallery -- one file, deferred
├── layouts/
│   ├── _default/
...
| *JS bundle*                | 5 files → 1 deferred =index.js=                                                                      |
...
- [X] Prettify internal links? Auto fill description for internal links to match title, so
  dynamically when any title changes it reflects everywhere?
  This needs ugly regex in code, but wonder how to handle override for specific.
